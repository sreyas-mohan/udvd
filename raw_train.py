...
def main(args):
    ...
    train_loader, valid_loader, test_loader = data.build_dataset(args.dataset, args.data_path, batch_size=args.batch_size, image_size=args.image_size, stride=args.stride, n_frames=args.n_frames, aug=args.aug)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch_id, (inputs, noisy_inputs) in enumerate(train_bar):
            ...
            train_psnr = utils.psnr(inputs[:, (mid*cpf):((mid+1)*cpf), :, :], outputs, normalized=False, raw=True)
            train_ssim = utils.ssim(inputs[:, (mid*cpf):((mid+1)*cpf), :, :], outputs, normalized=False, raw=True)
            train_meters["train_loss"].update(loss.item())
            train_meters["train_psnr"].update(train_psnr.item())
            train_meters["train_ssim"].update(train_ssim.item())

            if args.loss == "loglike":
                mean_psnr = utils.psnr(inputs[:, (mid*cpf):((mid+1)*cpf), :, :], mean_image, normalized=False, raw=True)
                mean_ssim = utils.ssim(inputs[:, (mid*cpf):((mid+1)*cpf), :, :], mean_image, normalized=False, raw=True)
            ...
            if (batch_id+1) % 5000 == 0:
                ...
                for sample_id, (sample, noisy_inputs) in enumerate(valid_bar):
                    with torch.no_grad():
                        ...
                        if args.loss == "loglike":
                            mean_psnr = utils.psnr(sample[:, (mid*cpf):((mid+1)*cpf), :, :], mean_image, normalized=False, raw=True)
                            mean_ssim = utils.ssim(sample[:, (mid*cpf):((mid+1)*cpf), :, :], mean_image, normalized=False, raw=True)
                            mean_meters["mean_psnr"].update(mean_psnr.item())
                            mean_meters["mean_ssim"].update(mean_ssim.item())
...
def get_args():
    ...
    parser.add_argument("--dataset", default="RawVideo", help="train dataset name")
